...
├── chat.py                      # ⭐ CLI 对话：流式 + 上下文记忆（全注释教学版）
│
...
这三个文件的注释密度差别很大，是**刻意**的 —— 请按难度递增阅读：

| 顺序 | 文件 | 行数 | 注释占比 | 定位 |
| --- | --- | --- | --- | --- |
| ① | `day01_llm_basics/my_chat_v1~v4.py` | 30～60 | 中等 | ⭐ **从这里开始**，四个递进小版本 |
| ② | `chat.py` | 655 | **67%** | 逐行讲解版，每处“为什么”都写了 |
| ③ | `llm_client.py` | 330 | 8% | 工程级封装，最后再读 |
| ④ | `main.py` | 126 | 1% | 三个 demo，对照 ① 看 |

> ⚠️ **关于注释密度**：
> `chat.py` 写成了 **67% 注释占比**，这在生产代码里是**反面教材**
> （正常 10～20% 就够），但在学习阶段是刻意的 ——
> 目的是一次性把"为什么这么写"讲透。
>
> 等你进入 Day 2～7，注释密度会逐步降到正常水平，
> 因为那时你已经不需要逐行解释了。
